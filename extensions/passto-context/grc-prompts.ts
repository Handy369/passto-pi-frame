...
import type { GoalStateDocument, ReflectorGoalContext, SummaryEntry } from "./types.js";
...
export function buildReflectorSubagentPrompt(
  conversation: string,
  currentGoalState: GoalStateDocument | null = null,
  goalContext: ReflectorGoalContext | null = null,
): string {
  return [
    "# 角色",
    ...
    "# 输入",
    "以下包含：",
    "1. 当前 round 的完整对话记录，包括用户需求、AI回复、工具调用和执行结果。",
    ...
    "必须优先相对 currentGoalState / goalContext 判断当前执行是否服务于当前目标链。",
    ...
    "}",
    "```",
    "如果没有值得沉淀或更新的原则，则输出 {\"principleOps\":[]}。",
    "原则要求：必须来自本轮真实执行经验，并与当前目标链相关，而不是空洞常识。",
    "",
    "# 约束",
    "- 总输出不超过 400 字",
    "- 不要复述对话内容",
