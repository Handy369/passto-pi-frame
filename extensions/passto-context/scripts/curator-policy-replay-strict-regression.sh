...
wait_for_pattern() {
  ...
  local attempts="${3:-40}"
  local delay="${4:-1}"
  local min_occurrences="${5:-1}"

  for _ in $(seq 1 "$attempts"); do
    capture "$name"
    local count
    count=$(rg -c "$pattern" "$LOG_DIR/$name.log" || true)
...
send_cmd "/ptc status"
wait_for_pattern status-before "PasstoContext Runtime Status" 30 1 1
wait_for_pattern status-before "proofSignals=|Latest Curator Summary" 60 1 1
...
run_reload
send_cmd "/ptc status"
wait_for_pattern status-after-reload "PasstoContext Runtime Status" 30 1 1
wait_for_pattern status-after-reload "proofSignals=|Latest Curator Summary" 60 1 1
...
artifact_round = 2 if artifact_round_2 is not None else None

if artifact is None:
    print('[FAIL] strict mode requires round-2 grc-curator-artifact, but none was found in jsonl', file=sys.stderr)
    sys.exit(1)
...
if not isinstance(goal_state, dict) or goal_state.get('version') != 2:
    print(f'[FAIL] strict mode requires round-2 artifact goalState.version = 2: {goal_state!r}', file=sys.stderr)
    sys.exit(1)
if not isinstance(certainty, dict) or not certainty.get('nextStepType'):
    print(f'[FAIL] strict mode requires round-2 artifact certaintyAssessment.nextStepType: {certainty!r}', file=sys.stderr)
    sys.exit(1)
if not isinstance(user_goal_tree, dict) or not str(user_goal_tree.get('currentFocusUserGoalId') or '').strip():
    print(f'[FAIL] round-2 artifact userGoalTree missing/invalid: {user_goal_tree!r}', file=sys.stderr)
    sys.exit(1)
if not isinstance(x_node_models, list) or not x_node_models or not isinstance(x_node_models[0], dict) or not str(x_node_models[0].get('currentFocusXNodeId') or '').strip():
    print(f'[FAIL] round-2 artifact xNodeModels missing/invalid: {x_node_models!r}', file=sys.stderr)
    sys.exit(1)
if not isinstance(latest_policy_projection, dict) or not latest_policy_projection.get('nextStepType'):
    print(f'[FAIL] strict mode requires round-2 artifact lastPolicyProjection.nextStepType: {latest_policy_projection!r}', file=sys.stderr)
    sys.exit(1)
if latest_transition is not None and (not isinstance(latest_transition, dict) or not str(latest_transition.get('label') or '').strip()):
    print(f'[FAIL] round-2 artifact latestGoalTransition invalid when present: {latest_transition!r}', file=sys.stderr)
    sys.exit(1)
...
if not isinstance(latest_runtime_proof, dict) or not str(latest_runtime_proof.get('targetXNodeId') or '').strip() or not str(latest_runtime_proof.get('proofStatus') or '').strip() or not str(latest_runtime_proof.get('proofMode') or '').strip():
    ...
    sys.exit(1)
if not isinstance(latest_proof_signals, list):
    print(f'[FAIL] strict mode requires round-2 artifact latestProofSignals to be an array: {latest_proof_signals!r}', file=sys.stderr)
    sys.exit(1)
if latest_runtime_proof.get('proofStatus') != 'passed' and (not latest_proof_signals or not isinstance(latest_proof_signals[0], dict) or not str(latest_proof_signals[0].get('type') or '').strip()):
    print(f'[FAIL] strict mode requires latestProofSignals[0].type when proofStatus is not passed: proof={latest_runtime_proof!r}, signals={latest_proof_signals!r}', file=sys.stderr)
    sys.exit(1)
...
def normalize_status_surface_text(text: str) -> str:
...
if effective_policy is None:
    print('[FAIL] strict mode requires effective policy before checking status surface', file=sys.stderr)
    sys.exit(1)
...
for name, block in [('before', status_before), ('after', status_after)]:
    ...
    if not latest_artifact_match:
        ...
        sys.exit(1)
    if latest_artifact_match.group(1) != str(expected_round):
        print(f'[FAIL] {name} status latest artifact round mismatch: {latest_artifact_match.group(1)!r}, expected={expected_round!r}', file=sys.stderr)
        sys.exit(1)
    if not curated_match:
        print(f'[FAIL] could not parse Last curated round from {name} status block', file=sys.stderr)
        sys.exit(1)
    if int(curated_match.group(1)) != expected_round:
        print(f'[FAIL] {name} status last curated round mismatch: {curated_match.group(1)!r}, expected={expected_round!r}', file=sys.stderr)
        sys.exit(1)
    if latest_transition is None:
        if '### Latest Goal Transition' in block:
            print(f'[FAIL] {name} status rendered Latest Goal Transition section while artifact transition is null', file=sys.stderr)
            sys.exit(1)
    else:
        if '### Latest Goal Transition' not in block:
            print(f'[FAIL] {name} status missing Latest Goal Transition section', file=sys.stderr)
            sys.exit(1)
        transition_label = str(latest_transition.get('label') or '').strip()
        if normalize_status_surface_text(transition_label) not in normalize_status_surface_text(block):
            print(f'[FAIL] {name} status missing transition label', file=sys.stderr)
            sys.exit(1)
    if '### Latest Policy Projection' not in block:
        print(f'[FAIL] {name} status missing Latest Policy Projection section', file=sys.stderr)
        sys.exit(1)
    expected_policy = f"nextStepType={effective_policy['nextStepType']}"
    if expected_policy not in block:
        print(f'[FAIL] {name} status missing nextStepType policy line', file=sys.stderr)
        sys.exit(1)
    if f"runtimeProof={effective_policy['dimensions']['runtimeProof']}" not in block:
        print(f'[FAIL] {name} status missing runtimeProof dimension', file=sys.stderr)
        sys.exit(1)
    if f"**Last Signal**: {signal['type']}" not in block:
        print(f'[FAIL] {name} status missing Last Signal prefix for {signal["type"]}', file=sys.stderr)
        sys.exit(1)
    if '### Latest Runtime Proof' not in block:
        print(f'[FAIL] {name} status missing Latest Runtime Proof section', file=sys.stderr)
        sys.exit(1)
    expected_runtime_proof_line = f"targetXNodeId={latest_runtime_proof['targetXNodeId']}, proofStatus={latest_runtime_proof['proofStatus']}, proofMode={latest_runtime_proof['proofMode']}"
    if expected_runtime_proof_line not in block:
        print(f'[FAIL] {name} status missing runtime proof line', file=sys.stderr)
        sys.exit(1)
    if latest_proof_signals:
        first_signal_type = latest_proof_signals[0]['type']
        if f'proofSignals={first_signal_type}' not in block and first_signal_type not in block:
            print(f'[FAIL] {name} status missing runtime proof signal type', file=sys.stderr)
            sys.exit(1)
...
if log_path.exists():
    ...
    if 'proofSource=curator-payload' not in recent_log_text and 'proofSource=x-node-fallback' not in recent_log_text:
        print('[FAIL] strict mode requires proofSource=curator-payload or proofSource=x-node-fallback in recent logs', file=sys.stderr)
        sys.exit(1)
...
print('[STRICT PASS] pre/post reload status are aligned with round-2 object-rich curator artifact')
PY

echo
echo '[OK] Curator certainty replay strict regression passed'
