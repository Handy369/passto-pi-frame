...
test('appendWidgetNotice appends active transient notice to widget status', () => {
  ...
  const text = appendWidgetNotice('Run:0 126.0k | 记:0+0 | 思:✗ | 理:✓ | 策:test', notice, 24, 1_000);
  assert.equal(text, 'Run:0 126.0k | 记:0+0 | 思:✗ | 理:✓ | 策:test | 梳理完成 + 目标更新');
});
...
test('appendWidgetNotice omits expired transient notice after 5 seconds window', () => {
  ...
  const text = appendWidgetNotice('Run:0 126.0k | 记:0+0 | 思:✗ | 理:✓ | 策:test', notice, 24, 5_000);
  assert.equal(text, 'Run:0 126.0k | 记:0+0 | 思:✗ | 理:✓ | 策:test');
