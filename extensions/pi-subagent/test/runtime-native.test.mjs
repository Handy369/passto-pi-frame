...
test("normalizeThinkingLevel preserves official pi thinking levels", () => {
...
test("buildPiArgs forwards minimal/xhigh without legacy downgrades", () => {
  ...
  assert.deepEqual(
    minimalArgs.slice(0, 7),
    ["--mode", "json", "-p", "--no-session", "--thinking", "minimal", "hello"],
  );

  const xhighArgs = buildPiArgs({
    options: {
      prompt: "hello",
      cwd: process.cwd(),
      thinking: "xhigh",
  ...
  assert.deepEqual(
    xhighArgs.slice(0, 7),
    ["--mode", "json", "-p", "--no-session", "--thinking", "xhigh", "hello"],
  );
});
